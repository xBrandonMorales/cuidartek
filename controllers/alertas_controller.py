...
@router.post("/", response_model=Alertas, dependencies=[Depends(require_medico)])
async def crear_alerta(alerta: AlertasCreate):
    try:
...
@router.get("/", response_model=List[Alertas], dependencies=[Depends(require_medico)])
...
async def actualizar_alerta(
    ...
    try:
        ...
        # Solo médicos o el paciente dueño de la alerta pueden actualizar
        if current_user["rol"] == "paciente":
            paciente = PacienteModel.get_by_usuario_id(current_user["id_usuario"])
...
@router.delete("/{alerta_id}", dependencies=[Depends(require_medico)])
async def eliminar_alerta(alerta_id: int):
    try:
        alerta_existente = AlertasModel.get_by_id(alerta_id)
        if not alerta_existente:
            raise HTTPException(status_code=404, detail="Alerta no encontrada")
